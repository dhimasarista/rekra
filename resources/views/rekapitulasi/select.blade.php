                            ...
                            @foreach ($config["form"] as $index => $form)
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="{{ $form["id"] }}" class="custom-select2 form-control" name="state"
                                            style="width: 100%; height: 38px;" {{ $form["is_disabled"] ? "disabled" : "" }}>
                                            @foreach ($form["options"] as $value)
                                                <option value="{{ $value["value"] }}" {{ $value["is_selected"] ? "selected" : "" }}> {{ Formatting::capitalize($value["name"]) }} </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @elseif ($form["type"] == "text")
                                        <div class="form-group">
                                            ...
                                            <input id="{{ $form["id"] }}" value="{{ $form["value"] ?? '' }}" type="text"
                                                class="form-control">
                                        </div>
                                    @endif
